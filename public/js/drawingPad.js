class DrawingPad {
  constructor(selector, doc, userInfo) {
    this.selector = selector;
    this.doc = doc;
    this.userInfo = userInfo;
    this.doc.submitSource = true;
    this.canvas = null;
    this.pencil = null;
    this.eraser = null;
    this.undoStack = [];
    ...
    this.groupedPresenceInfo = {};
    this.presenceGroupingTimeout = false;
  ...
  extractFromActiveSelection(callback) {
  ...
  addBoundingBoxToFabric() {
    fabric.BoundingBox = fabric.util.createClass(fabric.Rect, {
      ...
      initialize: function (options) {
        ...
        this.callSuper("initialize", { ...options, fill: "" });
        this.set({
          label: options.label || "",
        });
      },
      ...
      _render: function (ctx) {
        ...
        let top = -this.width / 2 + 0.5;
        let left = -(this.height - 20) / 2 + 10;
        ...
        ctx.strokeRect(top, left, this.width, this.height - 20);
        ctx.restore();
        ctx.fillStyle = this.color;
        if (this.showLabel !== false) {
          ctx.globalAlpha = 0.08;
          ctx.fillRect(top, left, this.width, this.height - 20);
          ctx.globalAlpha = 1;
        }

        const width = ctx.measureText(this.label).width;
        this.set({ width: Math.max(width + 10, this.width) });
  ...
  addTopCanvas() {
    const canvasTopElement = document.createElement("canvas");
    canvasTopElement.id = "canvasTop";
    Module.insertAfter(
      canvasTopElement,
      document.getElementById(this.selector)
    );

    this.canvasTop = new fabric.StaticCanvas("canvasTop");
    this.canvasTop.setDimensions({ width: 1920, height: 1080 });

    this.canvasTop.showBoundingBox = function (
      obj,
      label,
      strokeColor,
      showLabel
      ...
      this.add(obj.boundingBox);
    };

    this.canvasTop.hideBoundingBox = function (obj) {
      this.remove(obj.boundingBox);
    };
  }
  ...
  applyOps(ops) {
    ops.forEach((op) => {
      ...
      // this.showPresenceInfo(op);

      this.groupPreseceInfo(op);
    });
  }

  groupPreseceInfo(op) {
    if (!op.lastUpdatedBy) return;
    if (op.ld) return;

    let userId = op.lastUpdatedBy.userID;
    let username = op.lastUpdatedBy.username;

    // clean the op
    delete op.lastUpdatedBy;

    this.userInfo.colors[userId] =
      this.userInfo.colors[userId] || Module.tinycolor.random().toHexString();
    let index = op.p[1];
    if (op.lm !== undefined) {
      index = op.lm;
    }

    let obj = this.canvas.getObjects()[index];

    if (!this.presenceGroupingTimeout) {
      this.presenceGroupingTimeout = true;

      this.updateUserPresenceInfo(userId, username, obj);
  ...
  updateUserPresenceInfo(userId, username, obj) {
    if (!this.groupedPresenceInfo[userId])
      this.groupedPresenceInfo[userId] = {
        username,
        color: this.userInfo.colors[userId],
        objects: [obj],
      };
    else if (!this.groupedPresenceInfo[userId].objects.includes(obj)) {
      this.groupedPresenceInfo[userId].objects.push(obj);
    }
  }

  showPresenceInfo() {
    for (const userId in this.groupedPresenceInfo) {
      let presenceObj = this.groupedPresenceInfo[userId];
      let username = presenceObj.username;
      let color = presenceObj.color;
      let objects = presenceObj.objects;

      this.renderPresenceInfo(objects, username, color);
    }
  }

  renderPresenceInfo(objects, username, color) {
    let minLeft = this.canvas.width;
    let minTop = this.canvas.height;
    let maxRight = 0;
    let maxBottom = 0;
    let ids = [];
    ...
    objects.forEach((obj) => {
      ids.push(obj.id);
      let bounds;
      if (this.canvas.getActiveObjects().includes(obj)) {
        this.extractFromActiveSelection(() => {
          bounds = obj.getBoundingRect();
        });
      } else bounds = obj.getBoundingRect();
      minLeft = Math.min(bounds.left, minLeft);
      minTop = Math.min(bounds.top, minTop);
      maxRight = Math.max(bounds.left + bounds.width, maxRight);
      maxBottom = Math.max(bounds.top + bounds.height, maxBottom);
    });

    let presenceId = this.genHash(ids.sort().join(""));
    let dummyGroup = this.extendFabricObject(
      new fabric.Rect({
        top: minTop - 5,
        left: minLeft - 5,
        width: maxRight - minLeft + 10,
        height: maxBottom - minTop + 10,
        presenceId,
      })
    );
    ...
    if (objects.length > 1) {
      objects.forEach((obj) => {
        obj.presenceId = obj.id;
        obj.renderBoundingBox(username, color, false);
      });
    }

    dummyGroup.renderBoundingBox(username, color);
  }
  ...
  extendFabricObject(fabricObject) {
    const self = this || {};
    // extends the fabric object to add new persistent properties
    ...
    //add bounding box
    fabricObject.boundingBox = new fabric.BoundingBox();

    //add events to hide bounding box when object is modified
    let events = ["moving", "scaling", "rotating"];
    events.forEach((event) => {
      fabricObject.on(event, (options) => {
        self.canvasTop.hideBoundingBox(options.transform.target);
      });
    });

    fabricObject.boundingBoxTimeout = undefined;

    fabricObject.renderBoundingBox = function (label, strokeColor, showLabel) {
      if (self.visibleBoundingBoxes[this.presenceId]) {
        self.canvasTop.hideBoundingBox(
          self.visibleBoundingBoxes[this.presenceId]
        );
      }

      self.visibleBoundingBoxes[this.presenceId] = this;

      if (this.boundingBoxTimeout !== undefined) {
        self.canvasTop.hideBoundingBox(this);
        clearTimeout(this.boundingBoxTimeout);
      }

      self.canvasTop.showBoundingBox(this, label, strokeColor, showLabel);
      this.boundingBoxTimeout = setTimeout(() => {
        self.canvasTop.hideBoundingBox(this);
        if (
          self.visibleBoundingBoxes[this.presenceId].boundingBoxTimeout ===
          undefined
        )
          delete self.visibleBoundingBoxes[this.presenceId];
      }, [1500]);
    };
  ...
  handlePathDrawing() {
    this.canvas.on("path:created", (options) => {
      ...
      options.path = this.extendFabricObject(options.path);
      let path = options.path;

      // if its eraser path return
      if (path.globalCompositeOperation === "destination-out") return;

      path.id = new Module.ObjectID().toString();
      // convert the path object to a canvas object
      path = this.getFabricObject(path);
      const op = {
        p: ["canvas", this.doc.data.canvas.length],
        li: this.serializeFabricObject(path, true),
      };
  ...
  handleObjectModification() {
    this.canvas.on("object:modified", () => {
      this.extractFromActiveSelection((selObjects) => {
        let ops = selObjects.map((selObj) => {
          ...
          return {
            p: ["canvas", index, "props"],
            od: this.doc.data.canvas[index].props,
            oi: newProps,
          };
        });
    ...
    this.canvas.on("erasing:end", (opt) => {
      let ops = opt.targets.map((target) => {
        ...
        if (noOfEraserPaths === 1)
          ...
          return {
            p: ["canvas", index, "eraser", "objects", noOfEraserPaths - 1],
            li: eraserObj.objects[noOfEraserPaths - 1],
          };
        }
      });
      this.canvas.fire("canvas:changed", { op: ops });
  ...
  handleShapeDrawing() {
    ...
    this.canvas.on("mouse:down", (o) => {
      ...
      const mouseUpHandler = () => {
        ...
        const op = {
          p: ["canvas", this.doc.data.canvas.length],
          li: this.serializeFabricObject(shape, true),
        };
        this.canvas.fire("canvas:changed", { op });
      };
  ...
  addZoom() {
    this.canvas.on("mouse:wheel", (opt) => {
      ...
      // show the zoom level
      const zoomLevelDisplayDiv = document.querySelector(
        ".zoom-level-display-div "
      );
      ...
      for (let i = 0; i < positions.length; i++) {
        ...
        if (
          !(
            top < 0 ||
            left < 60 ||
            top + controlsHeight > canvasWrapper.clientHeight ||
            left + controlsWidth > canvasWrapper.clientWidth
          )
        ) {
          visible = true;
          break;
      ...
      if (activeObject.type !== "activeSelection") {
        ...
        if (strokeColorsDiv.querySelector(".color-selected"))
          strokeColorsDiv
            ...
            .classList.remove("color-selected");

        strokeColorsDiv
          .querySelector(`.fa-circle-dot[data-value="${strokeColor}"]`)
        ...
        if (activeObject.type === "path") {
          ...
          fillColorsDiv
            .querySelector(`.fa-circle[data-value="${fillColor}"]`)
            .classList.add("color-selected");
        }
      }
    };
      ...
      if (e.target && e.target.matches("i")) {
        ...
        if (target.type === "activeSelection") {
          ops = target._objects.map((obj) => {
            ...
            // submit the op
            return {
              p: ["canvas", index, "stroke"],
              od: this.doc.data.canvas[index].stroke,
              oi: color,
            };
          });
        } else {
      ...
      if (e.target && e.target.matches("i")) {
        ...
        if (target.type === "activeSelection") {
          target._objects.forEach((obj) => {
            ...
            ops.push({
              p: ["canvas", index, "fill"],
              od: this.doc.data.canvas[index].fill,
              oi: color,
            });
          });
        ...
        if (target.type === "activeSelection") {
          ...
          ops = selObjects.map((obj) => {
            ...
            if (action === "bring-to-front") {
              ...
              op = {
                p: ["canvas", index],
                lm: this.doc.data.canvas.length - 1,
              };
            } else {
              obj.sendToBack();
              // submit the op
  ...
  addToolbarLogic() {
    ...
    pointerBtn.addEventListener("click", () => {
      ...
      if (document.querySelector(".toolbar-btn-selected"))
        document
          .querySelector(".toolbar-btn-selected")
          .classList.remove("toolbar-btn-selected");
    ...
    editPenDiv
      .querySelector("#close-pen-options")
    ...
    eraserBtn.addEventListener("click", () => {
      if (!this.isErasing) {
        ...
        this.canvas.freeDrawingBrush.width =
          parseInt(strokeWidthRange.value, 10) || 1;
        this.isErasing = true;
        eraserBtn.classList.add("eraser-selected");
      } else {
        const color = editPenDiv
          .querySelector(".stroke-color-selected")
      ...
      .addEventListener("click", (e) => {
        if (e.target && e.target.matches("i")) {
          ...
          if (editPenDiv.querySelector(".stroke-color-selected"))
            editPenDiv
              .querySelector(".stroke-color-selected")
              .classList.remove("stroke-color-selected");
