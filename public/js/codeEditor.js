class CodeEditor {
  constructor(selector, doc, presence, collection) {
  ...
  submitPresence(localPresence) {
    ...
    Selection.on("changeCursor", () => {
      setTimeout(() => {
        ...
        selection.name = this.presence.userName;
  ...
  getSelectionRanges(start, end) {
    ...
    selectedRanges.push(
      new Range(start.row, start.column, start.row, lastColumn)
    );
    ...
    for (let i = 1; i < nLines - 1; i++) {
      ...
      selectedRanges.push(
        new Range(start.row + i, 0, start.row + i, lastColumn)
      );
    }
  ...
  addEditorFunctionality() {
    ...
    document.addEventListener("click", (e) => {
      if (!options.classList.contains("show-options")) return;

      if (
        e.target &&
        !e.target.matches(
          ".mode-selector, .mode-selector > *, .options .options > *"
        )
      ) {
        options.classList.remove("show-options");
      }
    ...
    outputBtn.addEventListener("click", (e) => {
      if (outputBtn.classList.contains("active-terminal-btn")) return;
    ...
    inputBtn.addEventListener("click", (e) => {
      if (inputBtn.classList.contains("active-terminal-btn")) return;
    ...
    document
      .querySelector(".close-terminal-btn")
    ...
    runBtn.addEventListener("click", async () => {
      ...
      const output = await this.piston.runCode(
        this.language,
